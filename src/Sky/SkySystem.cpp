...
namespace Sky {
	...
	void SkySystem::initialize(const Clouds::CloudsParameters& clouds_params,
		...
		setWeather(WeatherType::MostlyClear, 1);
	}
	...
	WeatherType SkySystem::pickNextWeather() const {
		...
		if (it == WeatherGraph.end() || it->second.empty()) return _currentWeather;
	...
	void SkySystem::setWeather(const WeatherType& preset, float transitionTime) {
		...
		case WeatherType::Clear:
			weather = WeatherMapGenerator::GenClear();
			std::cout << "(Clear)";
			break;
		case WeatherType::MostlyClear:
			weather = WeatherMapGenerator::GenMostlyClear(_dayTime);
			std::cout << "(Mostly Clear)";
			break;
		case WeatherType::PartlyCloudy:
			weather = WeatherMapGenerator::GenPartlyCloudy(_dayTime);
			std::cout << "(Partly Cloudy)";
            break;
		case WeatherType::MostlyCloudy:
			weather = WeatherMapGenerator::GenMostlyCloudy(_dayTime);
	...
	void SkySystem::updateWeather(double dt) {
		...
		std::uniform_real_distribution<float> dist(WEATHER_TRANSITION_MIN, WEATHER_TRANSITION_MAX);
		setWeather(next, dist(_rng));
