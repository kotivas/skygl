...
namespace Sky {
    namespace WeatherMapGenerator {
        ...
        std::vector<float> GenClear(float seed) {
        ...
        std::vector<float> GenMostlyClear(float seed) {
            std::vector<float> weather;
            weather.resize(Clouds::WEATHER_MAP_SIZE * Clouds::WEATHER_MAP_SIZE * 3);
        ...
        std::vector<float> GenPartlyCloudy(float seed) {
            std::vector<float> weather;
            weather.resize(Clouds::WEATHER_MAP_SIZE * Clouds::WEATHER_MAP_SIZE * 3);
            ...
            for (int x = 0; x < Clouds::WEATHER_MAP_SIZE; x++) {
                for (int y = 0; y < Clouds::WEATHER_MAP_SIZE; y++) {
                    ...
                    float coverage_worley = 1.0f - Noise::WorleyF1(fx * 24, fy * 24, 24, seed);
                    float coverage_perlin = Noise::PerlinFBM(3, fx, fy, 16, (unsigned)seed) * 0.5f + 0.5f;
                    float coverage = coverage_perlin * glm::smoothstep(0.3f, 1.0f, coverage_worley);

                    float cloudtype = glm::mix(0.0f, 0.35f, glm::smoothstep(0.1f, 0.4f, coverage)) * coverage_worley;
        ...
        std::vector<float> GenMostlyCloudy(float seed) {
            std::vector<float> weather;
            weather.resize(Clouds::WEATHER_MAP_SIZE * Clouds::WEATHER_MAP_SIZE * 3);
        ...
        std::vector<float> GenOvercast(float seed) {
            ...
            for (int x = 0; x < Clouds::WEATHER_MAP_SIZE; x++) {
                for (int y = 0; y < Clouds::WEATHER_MAP_SIZE; y++) {
                    ...
                    float c = Noise::PerlinFBM(3, fx, fy, 16, 1, (unsigned)seed) * 0.5f + 0.5f;

                    weather[coord + 0] = glm::smoothstep(0.2f, 1.0f, c); // coverage
        ...
        std::vector<float> GenStorm(float seed) {
            ...
            for (int x = 0; x < Clouds::WEATHER_MAP_SIZE; x++) {
                for (int y = 0; y < Clouds::WEATHER_MAP_SIZE; y++) {
                    ...
                    weather[coord + 0] = glm::smoothstep(0.1f, 1.0f, c); // coverage
